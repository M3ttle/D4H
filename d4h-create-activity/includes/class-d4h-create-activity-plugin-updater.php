...
final class Plugin_Updater {
	...
	public function fetch_latest_release(): ?array {
		...
		$token = function_exists( 'd4h_core_get_github_token' ) ? d4h_core_get_github_token() : '';
		$token = is_string( $token ) ? trim( $token ) : '';

		$request_args = array(
			'timeout'    => 15,
			'user-agent' => 'D4H-Create-Activity-WordPress-Plugin/' . $this->current_version,
		);
		if ( $token !== '' ) {
	...
	/**
	 * Pick a zip asset URL from a GitHub release payload.
	 *
	...
	private function get_package_url( array $release_data ): string {
		...
		foreach ( $assets as $asset ) {
			...
			if ( $url !== '' && ( $name === $slug . '.zip' || strpos( strtolower( $name ), 'create-activity' ) !== false ) ) {
				return $url;
			}
		}
		foreach ( $assets as $asset ) {
	...
	/**
	 * Register WordPress plugin update check against GitHub releases.
	 *
	 * @param array<string, mixed> $config
	 */
	public static function register_update_filter( array $config ): void {
		add_filter(
			...
			static function ( $value ) use ( $config ) {
				...
				$plugin_file = plugin_basename( D4H_CREATE_ACTIVITY_PLUGIN_FILE );
				$updater     = new self( $config, $plugin_file, D4H_CREATE_ACTIVITY_VERSION );
				$latest      = $updater->fetch_latest_release();
				if ( $latest === null || version_compare( D4H_CREATE_ACTIVITY_VERSION, $latest['version'], '>=' ) ) {
					return $value;
				}
				if ( ! is_object( $value ) ) {
					$value = new \stdClass();
				...
				$value->response[ $plugin_file ] = (object) array(
					'id'          => 'd4h-create-activity/d4h-create-activity.php',
					'slug'        => 'd4h-create-activity',
					'plugin'      => $plugin_file,
